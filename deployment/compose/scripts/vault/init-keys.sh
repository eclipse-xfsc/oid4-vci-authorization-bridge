...
echo 'create transit'
curl --header "X-Vault-Token: test" --request POST --data '{"type":"transit"}' http://localhost:8200/v1/sys/mounts/tenant_space
...
if curl --header "X-Vault-Token: test" --request GET localhost:8200/v1/sys/mounts | grep -q "tenant_space/"; then
...
curl --header "X-Vault-Token: test" \
     --request POST \
     --data "$KEY_DATA" \
     http://localhost:8200/v1/tenant_space/restore/signerkey

echo "Vault keys initialized successfully"

curl --header "X-Vault-Token: $VAULT_TOKEN" --request POST --data '{"type":"ecdsa-p256"}' http://localhost:8200/v1/tenant_space/keys/eckey 
